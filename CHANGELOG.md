...
+ [修复] 生图、图片编辑和 Gemini 生图请求增加 10 分钟超时，接口长时间不返回时节点会报超时而不是一直停在生成中。
+ [新增] 插件目录新增 ZCode 插件清单和本地 marketplace，ZCode 也能像 Codex 一样连接 Canvas Agent 操作画布。
+ [优化] 图片和文本上方的操作按钮（下载、创建副本、设为主图、设为主文本）改为鼠标移入后才显示，不再常驻遮挡画面内容。
+ [优化] 画布拖拽节点时，组投放检测和节点位置更新合并到同一帧计算，并按视口裁剪连线渲染，缓解节点和连线较多时的拖拽与缩放卡顿。
